  ...
  handleTextChange(e){ 
    ...
    if(e.target.type === 'text'){
      ...
      if(val === '' || val===null){
        ...
        formErrors[e.target.id] = "field can not be empty"; 
      }else if(val.length <= 10){
        console.log('length issue');
        console.log(val.length);
        console.log(e.target.id);
        formErrors[e.target.id] = "Length should be greater than 10"; 
      }else{
        console.log(true);
        console.log(val.length);
        formErrors[e.target.id] = '';
                  ...
                  if(d.type === 'text'){
                    return (
                      <li key={idx} class="listItem">
                          ...
                          <input className="contactform-input" id={`text${idx}`} name={d.label}  type={d.type}  onChange={this.handleTextChange} placeholder={d.placeholder} onLoad={this.handleTextChange} required={true}/>
                    ...
                    return (
                      <li className="listItem" key={idx}>
                          <label className="contactform-label" >{d.label}: </label>
                          <input className="contactform-input" id={idx} name={d.type} type={d.type} onChange={this.handleTextChange}  placeholder={d.placeholder} />
                          <span className="formerror">
                    ...
                    return (
                      <li className="listItem" key={idx}>
                          <label className="contactform-label">{d.label}: </label>
                          <input className="contactform-input" id={idx} name={d.label} type={d.type} onChange={this.handleTextChange} placeholder={d.placeholder} />
                      </li>)
                  }
                    ...
                    return (
                      <li className="listItem" key={idx}>
                          <label className="contactform-label">{d.label}: </label>
                          <textarea className="contactform-input" id={idx} name={d.label} onChange={this.handleTextChange} placeholder={d.placeholder} />
                      </li>)
                  }
                    ...
                    return (
                      <li className="listItem" key={idx}>
                          <label className="contactform-label">{d.label}: </label>
                          <select className="contactform-input" id={idx} name={d.label} onChange={this.handleTextChange}>
